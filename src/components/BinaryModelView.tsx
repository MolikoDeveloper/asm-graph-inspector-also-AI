  ...
  if (view === 'map') {
    return (
      <div className="binary-model-view">
        ...
        {summary.dependencies.length ? <div className="binary-dependency-list">{summary.dependencies.map((dependency, index) => <div className={`binary-dependency-row ${dependency.status}`} key={`${dependency.requestedName}:${dependency.role}:${dependency.requestedBy ?? 'root'}:${index}`} style={{ paddingLeft: `${10 + dependency.depth * 18}px` }}><span className="binary-dependency-status">{dependency.status === 'resolved' ? 'resolved' : dependency.status === 'permission-required' ? 'permission' : 'unresolved'}</span><strong>{dependency.requestedName}</strong><small>{dependency.role === 'interpreter' ? 'interpreter' : dependency.role === 'direct' ? 'direct DT_NEEDED' : `needed by ${dependency.requestedBy ?? 'dependency'}`} · {dependency.status === 'resolved' ? `via ${dependency.sourceName ?? dependency.fileName ?? 'global dependency'}${dependency.neededLibraries.length ? ` · needs ${dependency.neededLibraries.join(', ')}` : ''}` : dependency.evidence}</small></div>)}</div> : null}
