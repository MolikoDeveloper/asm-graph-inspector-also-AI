...
- [x] Detect objdump/disassembly text by content instead of trusting the `.asm` extension.
- [x] Route objdump text to a dedicated importer/parser, never to the NASM source parser.
- [ ] Treat dump text as analysis evidence/annotation when authoritative binary bytes are also available.
- [x] Never use objdump text in place of ELF bytes for binary execution.
...
- [x] Audit executable bytes of the materialized interpreter and recursive Global Dependencies as well as the main guest image; dependency AVX is inventoried without false rejection of optional glibc/multiarch/GNU IFUNC implementations, while positively decoded unsupported ISA exactly at the mandatory `PT_INTERP` entry is blocking and observed runtime `SIGILL` remains authoritative for dispatched paths.
...
- [x] Define a generic artifact compatibility report covering machine architecture, ELF class/endianness, CPU ISA, interpreter, dependency closure, required symbol versions, runtime dependency ISA inventory, unsupported syscalls/environment services and observed runtime traps.
...
Runtime dependency ISA inventory uses only lightweight ELF64 program headers plus executable `PT_LOAD` bytes; it deliberately does not invoke full symbol/relocation/unwind/function discovery for libc/ld.so. Whole-image unsupported instructions in runtime DSOs are evidence, not proof of incompatibility, because CPU dispatch/IFUNC can keep them unreachable. The currently safe static blocker is an unsupported instruction exactly at the selected ELF interpreter entry. Anything reached later is decided by observed execution: a fatal Blink signal records fresh registers, RIP, code bytes and runtime-image attribution so the Debug Console can identify the exact module/instruction that actually failed.

The inspector-side compatibility gate is deliberately fail-closed only on positively decoded unsupported instructions on a path proven mandatory by the current evidence model. Bytes Capstone cannot decode are reported as skipped evidence rather than guessed to be AVX. The inspector must diagnose the binary that it was actually given, resolve the libraries it actually requests, and report precisely which sandbox capability blocks execution. It must not rewrite the guest or infer compatibility from the compiler/language that produced it.
